...
let PeriodsService = class PeriodsService {
    ...
    getWeekLabelKey(orderDate, getDateWeekNumber) {
    ...
    getWeekLabelsKeys(dateRanges, getWeekFunction) {
        ...
        const keys = [];
        const labels = [];
        const labelPrefix = this._translate('DASHBOARD_VIEW.CHARTS.LABELS.WEEK');
        while (true) {
    ...
    getWeekDays() {
        const translationWeekDaysPrefix = 'DASHBOARD_VIEW.CHARTS.LABELS.WEEKDAYS';
        const labels = [
    ...
    getDatesLabelsKeys(dateRanges) {
        ...
        while (true) {
            ...
            keys.push(this._getFormattedDate(currentDay, currentMonth, currentYear));
            labels.push(this._getFormattedDate(currentDay, currentMonthLabel, currentYear));
    ...
    getYearLabels(yearsRange) {
        let { from, to } = yearsRange;
        if (from === Number.MAX_SAFE_INTEGER) {
            from = to;
        }
        let labels = this._generateYearLabels(from, to);
        if (labels.length === 1) {
    ...
    _generateYearLabels(yearFrom, yearTo) {
        const labels = yearFrom === yearTo
            ? [`${yearFrom}`]
            : [
                `${yearFrom}`,
    ...
    _getFormattedDate(day, month, year) {
